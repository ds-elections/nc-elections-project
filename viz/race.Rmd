...
nc_race <- nc %>%
  filter(!is.na(race_code), 
         ...
         !is.na(ethnic_code),
                age < 122) %>%
  select(race_code, ethnic_code, age) %>%
  collect(n = Inf)

nc_race <- mutate(nc_race, race_code = if_else(ethnic_code == "HL", true = "Hispanic", 
                        false = race_code, missing = race_code))
```

```{r, 2012 viz}
full_data <- rbind(cces_2012_viz, cces_2014_viz, cces_2016_viz, cps_2012_viz, cps_2014_viz)

full_data %>%
  filter(source %in% c("cces2012", "cps2012")) %>%
  ggplot(aes(x = race, color = source)) +
  geom_bar(position = "dodge")
```

```{r, 2014}
full_data %>%
  filter(source %in% c("cces2014", "cps2014")) %>%
  ggplot(aes(x = race, color = source)) +
  geom_bar(position = "dodge")
```

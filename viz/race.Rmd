...
nc_race <- nc %>%
  filter(!is.na(race_code), 
         ...
         !is.na(ethnic_code),
                age < 122) %>%
  select(race_code, ethnic_code, age, registr_dt, new_id) %>%
  collect(n = Inf)

nc_race <- nc_race %>%
  mutate(race_code = if_else(ethnic_code == "HL", true = "Hispanic", 
                        false = race_code, missing = race_code)) %>%
  mutate(voted = if_else(election_lbl == "2012elect", true = "Voted2012",
                         false = election_lbl, missing = "null")) %>%
  mutate(voted = if_else(election_lbl == "2014elect", true = "Voted2014",
                         false = voted, missing = voted)) %>%
  mutate(voted = if_else(election_lbl == "2016elect", true = "Voted2016",
                         false = voted, missing = voted))
...
nc_race_2016 <- nc_race %>%
  filter(registr_dt <= as.Date("2016-10-18")) %>%
...
#rm(c("cces_2012_viz", "cces_2014_viz", "cces_2016_viz", "cps_2012_viz", "cps_2014_viz", "nc_race_2012", "nc_race_2014", "nc_race_2016"))

#to plot actual voting pop, filter(voted = "Voted")

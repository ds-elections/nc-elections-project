...
```{r, 2012 viz}
full_data <- rbind(cces_2012_viz, cces_2014_viz, cces_2016_viz, cps_2012_viz, cps_2014_viz, nc_race_2012, nc_race_2014, nc_race_2016)

to_plot_2012 <- full_data %>%
  filter(source %in% c("cces_2012_viz", "cps_2012_viz", "nc_race_2012")) %>%
  group_by(source) %>%
  summarise(n = n()) %>% 
...
ggplot(to_plot_2012, aes(source, prop, fill = race)) +
 ...
 coord_cartesian(ylim = c(0,.25)) +
  geom_text(aes(label = prop), vjust=-.05)
```

```{r, 2014}
to_plot_2014 <- full_data %>%
  filter(source %in% c("cces_2014_viz", "cps_2014_viz", "nc_race_2014")) %>%
  group_by(source) %>%
  summarise(n = n()) %>% 
  right_join(full_data, by = "source") %>% 
  group_by(source, race) %>% 
...
to_plot_2016 <- full_data %>%
  filter(source %in% c("cces_2016_viz", "nc_race_2016")) %>%
  group_by(source) %>%
  summarise(n = n()) %>% 
  right_join(full_data, by = "source") %>% 
  group_by(source, race) %>% 
